...
- `aiflow run` still has no SIGINT/AbortController wiring, and the gap is three-part, not a single small change. `runRalphLoop`'s `signal` parameter is plumbed through `engine.ts`'s `runPipelineOnce` (which accepts `signal` as its 7th positional parameter) and is exercised by tests, but the chain stops there: (1) `src/commands/run.ts`'s exported `runCommand(cwd, pipelineName, overrides)` has no `signal` parameter at all to receive one — it would need one added, either as a new parameter or on `RunCommandOverrides`; (2) `runCommand`'s call to `runPipelineOnce(...)` never supplies that 7th positional `signal` argument, so even an internally-available signal wouldn't reach the engine; and (3) `src/cli.ts`'s `run` command action has no SIGINT handling at all — unlike `aiflow watch`, which does register `process.once("SIGINT", ...)` with a real `AbortController` and pass its `.signal` through. That gap pre-dates this plan and is out of scope here; the follow-up requires all three changes together (thread `signal` through `runCommand` → `runPipelineOnce`, then wire SIGINT in `cli.ts`'s `run` action), not just the `cli.ts` piece.
